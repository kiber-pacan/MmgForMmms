...
const std::vector<Vertex> vertices = {
    {{-0.5f, -0.5f}, {1.0f, 0.0f, 0.25f}},
    {{0.5f, -0.5f}, {0.75f, 1.0f, 0.25f}},
    {{0.5f, 0.5f}, {0.25f, 0.0f, 0.75f}},
    {{-0.5f, 0.5f}, {0.34f, 0.55f, 0.9f}}
};
...
//Debug file paths
const char* VertexShader = "main/shaders/basic/vert.spv";
const char* FragShader = "main/shaders/basic/frag.spv";
const char* DevTestTexture = "main/textures/dev/devtest.png";
...
private:
    ...
    //Destroying swapchain buffers, images and then destroying the swapchain
    ...
    //Creating image view for texture to access it
    ...
    void createTextureImage() {
        ...
        stbi_uc* pixels = stbi_load(DevTestTexture, &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
    ...
    //Creating image with specific properties
    ...
    //Creating uniform buffer for storing data avaliable for all shaders
    ...
    //Creating descriptors for passing data in shaders
    ...
    //Creating buffer that stores commands that should be done on GPU side
    ...
    //Finding memory type that corresponds certain properties
    ...
    //Generic method for creating different types of buffers
    ...
    //Pool for storing command buffers
    ...
    //Recording commands to command buffer
    ...
    void createGraphicsPipeline() {
        auto vertShaderCode = readFile("main/shaders/basic/vert.spv");
        auto fragShaderCode = readFile("main/shaders/basic/frag.spv");
    ...
    //Get swap chain height and width (extent)
    VkExtent2D chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
        char buffer[26];

        if (capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max()) {
            sprintf_s(buffer, sizeof(buffer), "Swap chain width is: %d", capabilities.currentExtent.width);
            printf("\x1b[38;2;0;98;255m%s\x1b[0m\n", buffer);

            sprintf_s(buffer, sizeof(buffer), "Swap chain height is: %d", capabilities.currentExtent.height);
            printf("\x1b[38;2;0;98;255m%s\x1b[0m\n", buffer);
    ...
    //Find out if device have a Indices family and if it has it the return queue
    ...
    const char* boolToChar(bool value) {
        return value ? "\x1b[38;2;0;209;10mtrue" : "false";
    }

    //Is device suitable for Vulkan
    ...
    //Creating logical device for using the gpu
    ...
    //Messenger for debugging vkCreateInstance() vkDestroyInstance()
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        ubo.model = glm::rotate(glm::mat4(1.0f), sin(time * 4) * glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
    ...
    //Get Vulkan api extensions(addons)
    ...
    void createInstance() {
        ...
        appInfo.pApplicationName = "makcyta2009";
